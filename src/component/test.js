              ...
              {updatedDetailsOfNotifications?.map((notify) => (
                <div key={notify.notificationId}>
                  {notify?.notificationType === 'like' ? (
                    ...
                    <div>
                      {notify?.receiverId === userInfo?._id ? (
                        <Link
                          to={`/user/${notify?.senderId}`}
                          className={
                        ...
                        <Link
                          to={`/user/${notify?.receiverId}`}
                          className={
                            notify.isReadBySender === false
                              ? 'px-4 p-2 text-decoration-none text-black d-flex unread align-items-center gap-3 '
                              : 'px-4   d-flex align-items-center  p-2 gap-3  text-decoration-none text-black '
                          }
                          key={notify.notificationId}
                          onClick={() =>
                            handleViewFriendRequest(
                        ...
                        >
                          <div>
                            ...
                            <img
                              src={`http://localhost:4000/${notify?.receiverImgUrl}`}
                              alt=""
                              style={{
                                width: '40px',
                                height: '40px',
                          ...
                          <span className="d-flex flex-column">
                            <Link
                              to={`/user/${notify?.receiverId}`}
                              className="text-decoration-none gap-2 d-flex align-items-center text-black"
                              style={{ opacity: '0.7' }}
                            >
                              {' '}
                              you and <strong>{notify?.receiverName}</strong>
                              are now friends
                            </Link>{' '}
                            <span
                              className="text-start"
